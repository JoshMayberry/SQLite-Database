...
class Database():
	...
	def formatSchema(self, relation, schema = None, applyChanges = None, autoPrimary = False, notNull = None, 
		...
		for foreign_item in self.ensure_container(foreign):
			if (foreign_item is not None):
				foreignList.append(self.formatForigen(foreign_item))
	...
	def insertForeign(self, relation, attribute, value, foreignNone = False):
		...
		print("@insertForeign.1", relation, attribute, value)
		...
		print("@insertForeign.2", foreign_id)
		return foreign_id
	...
	def configureForeign(self, relation, attributeDict):
		"""Configures the portions of an SQL statement that deal with foreign keys."""

		attributeList, valueList = zip(*attributeDict.items())
		if (relation not in self.foreignKeys_catalogue):
			return attributeList, valueList, ("?" for value in valueList)

		foreignList = []
		catalogue = self.foreignKeys_catalogue[relation]
		for attribute in attributeList: 
			if (attribute not in catalogue):
				foreignList.append("?")
				continue

			foreign_relation, foreign_attribute = catalogue[attribute]
			foreignList.append(f"(SELECT [{self.getPrimaryKey(foreign_relation)}] FROM [{foreign_relation}] WHERE ([{foreign_attribute}] = ?))")
		
		# print("@configureForeign.1", attributeList, valueList, foreignList)

		return attributeList, valueList, foreignList

	def formatForigen(self, foreign, allowDelete = None, allowUpdate = True):
		"""Formats a forigen key
		Use: http://www.sqlitetutorial.net/sqlite-foreign-key/
		Use: https://www.sqlite.org/foreignkeys.html#fk_actions

		allowDelete (bool) - Determines what happens if the foreign key is deleted while children are still referencing it
		...
		allowUpdate (bool) - Determines what happens if the foreign key is updated while children are still referencing it
		...
		"""
		foreignInfo = []
		for attribute, (foreign_relation, foreign_attribute) in foreign.items():
			command = f"FOREIGN KEY ([{attribute}]) REFERENCES [{foreign_relation}] ([{foreign_attribute}])"
			command += f" ON DELETE { {True: 'CASCADE', False: 'SET DEFAULT', None: 'RESTRICT'}[allowDelete] }"
			command += f" ON UPDATE { {True: 'CASCADE', False: 'SET DEFAULT', None: 'RESTRICT'}[allowUpdate] }"
			foreignInfo.append(command)
	...
	def executeCommand_add(self, relation, *args, add = True, **kwargs):
		...
		command = f"SELECT DISTINCT {', '.join(f'[{attribute}]' for attribute in attributeList)} FROM [{relation}]"
	...
	def executeCommand(self, command, valueList = None, hackCheck = False, printError_command = True, 
		filterNone = False, valuesAsSet = False, transpose = False, variableLength = False, default = None, 
		exclude = None, excludeFunction = None, include = None, includeFunction = None, 
		attributeFirst = False, filterTuple = False, rowsAsList = False, forceAttribute = None, forceTuple = None,
		...
		def yieldValue_result(row, i = None, returnFlag = False):
			...
			if (not resultKeys):
				...
				if ((not yieldedSomething) and (not variableLength)):
					yield default
				return
			...
			if (i is None):
				for attribute, value in zip(resultKeys, row):
					...
					yield self.formatValue(value, attribute, relation, returnNull = returnNull, checkForeign = checkForeign, formatter = formatter)
					yieldedSomething = True
				if ((not yieldedSomething) and (not variableLength)):
					yield default
		...
		if (resultKeys):
			...
			rows = tuple(yieldRow(command, valueList))
			_forceTuple = forceTuple or (len(rows) > 1)
			_forceAttribute = forceAttribute or (len(resultKeys) > 1)

			# print("@execute.0")
			# print("   -- resultKeys:", resultKeys)
			# print("   -- attributeFirst:", attributeFirst)
			# print("   -- forceTuple:", forceTuple, _forceTuple)
			# print("   -- forceAttribute:", forceAttribute, _forceAttribute)
			# print("   -- rowsAsList:", rowsAsList)
			# print("   -- filterTuple:", filterTuple)
			# print("   -- valuesAsSet:", valuesAsSet)
			# print("   -- filterNone:", filterNone)
			# print("@execute.1", rows)
			...
			if (attributeFirst):
				if (_forceTuple):
					if (not rows):
						...
						# ((1, "Lorem"), (2, "Ipsum")) -> {id: [1, 2], name: ["Lorem", "Ipsum"]}
	...
	def openDatabase(self, fileName = "myDatabase", *args, applyChanges = True, multiThread = False, connectionType = None, 
		...
		if (self.isSQLite):
			...
			self.connection = MySQLdb.connect(passwd = password, db = fileName)
	...
	def clearRelation(self, relation = None, applyChanges = None):
		...
		#Update internal foreign schema catalogue
		self.updateInternalforeignSchemas()
		self.subtractForeignUse(relation = relation, n = None)
	...
	def renameRelation(self, relation, newName, applyChanges = None):
		...
		if (relation != newName):
			...
			#Update internal foreign schema catalogue
			self.updateInternalforeignSchemas()
			self.renameForeignUse(relation, newName)
	...
	def createRelation(self, relation, schema = {}, applyChanges = None, autoPrimary = True, 
		...
		if (isinstance(schema, str)):
			...
			self.executeCommand(command)
			foreign = re.search("foreign", raw_sql, re.IGNORECASE)
		else:
			#Format schema
			commandList = []
			if (autoPrimary):
				commandList.append(self.getDefinition("id", dataType = int, autoPrimary = True))
			commandList.append(self.formatSchema(relation, schema = schema, applyChanges = applyChanges, autoPrimary = False, notNull = notNull, 
		...
		if (foreign):
			_index = self.getPrimaryKey(relation)
			for foreign_item in self.ensure_container(foreign):
				if (not foreign_item):
					continue

				self.applyForeign(relation, foreign_item)
		...
		self.updateInternalforeignSchemas()
		if (foreign):
			self.updateForeignUses()

	def applyForeign(self, relation, foreign):
		for attribute, (foreign_relation, foreign_attribute) in foreign.items():
			self.createIndex(relation, attribute, f"fkIndex_{relation}_{attribute}", noReplication = True)

			# foreign_index = self.getPrimaryKey(foreign_relation)
			# selectCommand =   f"SELECT [{foreign_index}] FROM [{foreign_relation}] WHERE ([{foreign_attribute}] = {{}}.[{attribute}])"
			# selectCommand_2 = f"SELECT [{foreign_index}] FROM [{foreign_relation}] WHERE ([{foreign_attribute}] = {{}}.[{attribute}] and [{foreign_attribute}] != {{}}.[{attribute}])"

			# #I am brand new, so I do not have anything
			# self.removeTrigger(f"fkTrigger_insert_{relation}_{attribute}")
			# reaction = f"INSERT OR IGNORE INTO [{foreign_relation}] ({foreign_attribute}) VALUES (NEW.[{attribute}]);\n"
			# reaction +=    f"UPDATE [{relation}] SET [{attribute}] = ({selectCommand.format('NEW')}) WHERE (rowid = NEW.rowid);"
			# command = f"CREATE TRIGGER fkTrigger_insert_{relation}_{attribute} AFTER INSERT ON [{relation}] FOR EACH ROW \nBEGIN \n{reaction} \nEND;"
			# self.executeCommand(command)

			# # #The key I want exists
			# # self.removeTrigger(f"fkTrigger_update_exists_{relation}_{attribute}")
			# # reaction = f"UPDATE [{relation}] SET [{attribute}] = ({selectCommand.format('NEW')}) WHERE (rowid = NEW.rowid);"
			# # command = f"CREATE TRIGGER fkTrigger_update_exists_{relation}_{attribute} AFTER UPDATE ON [{relation}] "
			# # command +=    f"FOR EACH ROW WHEN (EXISTS ({selectCommand.format('NEW')})) \nBEGIN \n{reaction} \nEND;"
			# # self.executeCommand(command)

			# # #The key I want does not exist
			# # self.removeTrigger(f"fkTrigger_update_new_{relation}_{attribute}")
			# # reaction = f"INSERT OR IGNORE INTO [{foreign_relation}] ({foreign_attribute}) VALUES (NEW.[{attribute}]);\n"
			# # reaction +=    f"UPDATE [{relation}] SET [{attribute}] = ({selectCommand.format('NEW')}) WHERE (rowid = NEW.rowid);"
			# # command = f"CREATE TRIGGER fkTrigger_update_new_{relation}_{attribute} AFTER UPDATE ON [{relation}] "
			# # command +=    f"FOR EACH ROW WHEN (NOT EXISTS ({selectCommand.format('NEW')})) \nBEGIN \n{reaction} \nEND;"
			# # self.executeCommand(command)
			...
			# #I do not currently have a foreign key; The key I want can exist or not
			# self.removeTrigger(f"fkTrigger_update_new_{relation}_{attribute}")
			# reaction = f"INSERT OR IGNORE INTO [{foreign_relation}] ({foreign_attribute}) VALUES (NEW.[{attribute}]);\n"
			# reaction +=    f"UPDATE [{relation}] SET [{attribute}] = ({selectCommand.format('NEW')}) WHERE (rowid = NEW.rowid);"
			# command = f"CREATE TRIGGER fkTrigger_update_new_{relation}_{attribute} AFTER UPDATE ON [{relation}] "
			# command +=    f"FOR EACH ROW WHEN (NOT EXISTS ({selectCommand.format('OLD')})) \nBEGIN \n{reaction} \nEND;"
			# self.executeCommand(command)

			# # #I currently have a foreign key; The one I want exists
			# # self.removeTrigger(f"fkTrigger_update_exists_{relation}_{attribute}")
			# # reaction = f"UPDATE [{relation}] SET [{attribute}] = ({selectCommand.format('NEW')}) WHERE (rowid = NEW.rowid);"
			# # command = f"CREATE TRIGGER fkTrigger_update_exists_{relation}_{attribute} AFTER UPDATE ON [{relation}] "
			# # command +=    f"FOR EACH ROW WHEN (EXISTS ({selectCommand.format('NEW')})) \nBEGIN \n{reaction} \nEND;"
			...
			# # #I currently have a foreign key; The one I want does not exist; I am the only one using it
			# # self.removeTrigger(f"fkTrigger_update_onlyOne_{relation}_{attribute}")
			# # reaction = f"UPDATE [{foreign_relation}] SET [{foreign_attribute}] = (NEW.[{attribute}]) WHERE ([{foreign_attribute}] = OLD.[{attribute}]);\n"
			# # reaction +=    f"UPDATE [{relation}] SET [{attribute}] = ({selectCommand.format('NEW')}) WHERE (rowid = NEW.rowid);\n"
			# # command = f"CREATE TRIGGER fkTrigger_update_onlyOne_{relation}_{attribute} AFTER UPDATE ON [{relation}] "
			# # command +=    f"FOR EACH ROW WHEN ((EXISTS ({selectCommand.format('NEW')})) AND (NOT EXISTS "
			# # command +=    f"(SELECT [{attribute}] FROM [{relation}] WHERE [{attribute}] = NEW.[{attribute}]))) \nBEGIN \n{reaction} \nEND;"
			# # self.executeCommand(command)

			# # #I currently have a foreign key; The one I want does not exist; I am sharing it with others
			# # self.removeTrigger(f"fkTrigger_update_onlyOne_{relation}_{attribute}")
			# # reaction = f"INSERT OR IGNORE INTO [{foreign_relation}] ({foreign_attribute}) VALUES (NEW.[{attribute}]);\n"
			# # reaction +=    f"UPDATE [{relation}] SET [{attribute}] = ({selectCommand.format('NEW')}) WHERE (rowid = NEW.rowid);"
			# # command = f"CREATE TRIGGER fkTrigger_update_onlyOne_{relation}_{attribute} AFTER UPDATE ON [{relation}] "
			# # command +=    f"FOR EACH ROW WHEN ((EXISTS ({selectCommand.format('NEW')})) AND (EXISTS "
			# # command +=    f"(SELECT [{attribute}] FROM [{relation}] WHERE [{attribute}] = NEW.[{attribute}]))) \nBEGIN \n{reaction} \nEND;"
			# # self.executeCommand(command)
	...
	def addAttribute(self, relation, attribute, dataType = str, default = None, notNull = None, 
		...
		command += self.formatSchema(relation, schema = {attribute: dataType}, notNull = {attribute: notNull}, primary = {attribute: primary}, 
			autoIncrement = {attribute: autoIncrement}, unsigned = {attribute: unsigned}, unique = {attribute: unique}, 
	...
	def addTuple(self, myTuple = None, applyChanges = None, autoPrimary = False, notNull = False, foreignNone = False,
		...
		for relation, rows in myTuple.items():
			...
			for attributeDict in rows:
				...
				# attributeList, valueList = zip(*attributeDict.items())
				attributeList, valueList, foreignList =  self.configureForeign(relation, attributeDict)
				# if (not checkForeign):
				# else:
				# 	valueList = []
				# 	attributeList = []
				...
				if (self.isAccess):
					command = f"INSERT INTO [{relation}] ({', '.join(f'[{attribute}]' for attribute in attributeList)}) VALUES ({', '.join(foreignList)})"
				else:
					command = f"INSERT { {True: 'OR REPLACE ', False: '', None: 'OR IGNORE '}[unique] }INTO [{relation}] ({', '.join(f'[{attribute}]' for attribute in attributeList)}) VALUES ({', '.join(foreignList)})"
	...
	def changeTuple(self, myTuple, nextTo, value = None, forceMatch = None, applyChanges = None, checkForeign = True, updateForeign = None, **locationKwargs):
		...
		for relation, attributeDict in myTuple.items():
			# print("@changeTuple.0", f"{time.perf_counter() - startTime:.6f}")
			if (not isinstance(attributeDict, dict)):
				if (isinstance(attributeDict, (int, str, float))):
			...
			_attributeList, newValues = attributeList, valueList
			...
			# 	foreignKeys, foreignConnections = zip(*((attribute, connection) for attribute, connection in self.foreignKeys_catalogue[relation].items() if (attribute in attributeDict)))

			# 	selectCommand = f"""SELECT {
			# 	", ".join(f"[{relation}].[{attribute}]" for attribute in foreignKeys)}, {
			# 	", ".join(f"[{foreign_relation}].[{foreign_attribute}]" for foreign_relation, foreign_attribute in foreignConnections)
			# 	} FROM [{relation}] {
			# 	" ".join(f"LEFT JOIN [{foreign_relation}] ON [{foreign_attribute}]" for foreign_relation, foreign_attribute in foreignConnections)}"""
			# 	print("\n@changeTuple.2.1", selectCommand)
			# 	print("@changeTuple.2.2", self.executeCommand(selectCommand), "\n")

			# foreignKeys_catalogue = self.foreignKeys_catalogue[relation]

			# for attribute, value in attributeDict.items():
			# 	if (attribute in foreignKeys_catalogue):
			# 		foreign_relation, foreign_attribute = foreignKeys_catalogue[attribute]

			# 		selectCommand = f"SELECT [{foreign_relation}].[{self.getPrimaryKey(foreign_relation)}] FROM [{foreign_relation}] WHERE [{attribute}] = ?"
			# 		command = f"UPDATE {relation} SET [{attribute}] = ({selectCommand}) WHERE EXISTS ({selectCommand})"
			# 		print("\n@changeTuple.2.1", command, (value,))
			# 		# print("@changeTuple.2.2", self.executeCommand(command))
			# 		sys.exit()


			# 		# command = f"""UPDATE {relation} SET [{relation}].[{attribute}] = [{foreign_relation}].[{
			# 		# self.getPrimaryKey(foreign_relation)}] WHERE [{relation}].[{attribute}] = [{foreign_relation}].[{foreign_attribute}]"""

			# 		# print("@changeTuple.2.3", command)

			# print("@changeTuple.3", f"{time.perf_counter() - startTime:.8f}")
			...
			# # startTime = time.perf_counter()
			# # command = f"UPDATE [{relation}] SET {', '.join(f'{attribute} = ?' for attribute in attributeList)}"
			# # if (locationInfo):
			# # 	command += f" WHERE ({locationInfo})"
			# # print(command, [*yieldValue(valueList), *locationValues])
			# # self.executeCommand(command, (*yieldValue(valueList), *locationValues))
			# # print("@changeTuple.1", f"{time.perf_counter() - startTime:.6f}")
	...
	# 		for attribute, newValue in zip(attributeList, yieldValue(valueList)):
	# 			if (attribute in oldRows):
	# 				startTime = time.perf_counter()
	# 				newValue = self.changeForeign(relation, attribute, newValue, oldRows[attribute], updateForeign = updateForeign)
	# 				# print("@changeTuple.yieldChange", f"{time.perf_counter() - startTime:.6f}", relation, attribute, newValue, oldRows[attribute])
	# 			yield attribute, newValue
	...
	# 				filterNone = True, relation = relation, checkForeign = False, attributeFirst = True, forceTuple = True, forceAttribute = True)
	# 			incrementForeign = True
	# 		else:
	# 			incrementForeign = False
					
	# 		# print("@changeTuple.4.1", f"{time.perf_counter() - startTime:.6f}")
	# 		_attributeList, newValues = zip(*yieldChange(attributeList, valueList))
	# 		# print("@changeTuple.4.2", f"{time.perf_counter() - startTime:.6f}")
	# 		command = f"UPDATE [{relation}] SET {', '.join(f'{attribute} = ?' for attribute in _attributeList)}"
	# 		if (locationInfo):
	# 			command += f" WHERE ({locationInfo})"
	# 		# print("@changeTuple.5.1", f"{time.perf_counter() - startTime:.6f}")
	# 		self.executeCommand(command, (*newValues, *locationValues))
	# 		# print("@changeTuple.5.2", f"{time.perf_counter() - startTime:.6f}")

	# 		if (incrementForeign):
	# 			newRows = self.executeCommand(selectCommand, affected[0], resultKeys = foreign_attributeList, filterNone = True, 
	# 				relation = relation, checkForeign = False, attributeFirst = True, variableLength = True, forceTuple = True, forceAttribute = True)
	...
	# 		# print("@changeTuple.6", f"{time.perf_counter() - startTime:.6f}")
	# 		self.autoSave(applyChanges)
	# 	# print("@changeTuple.7", f"{time.perf_counter() - startTime:.6f}")
	...
	def getValue(self, myTuple, nextTo = None, orderBy = None, limit = None, direction = None, alias = None, 
		...
		for _myTuple, _nextTo in myTuple:
			...
			for relation, attributeList in _myTuple.items():
				...
				# if (relation in self.foreignKeys_catalogue):
				# 	foreignKeys, foreignConnections = zip(*((attribute, connection) for attribute, connection in self.foreignKeys_catalogue[relation].items() if (attribute in attributeList)))

				# 	selectCommand = f"""SELECT {
				# 	", ".join(f"[{relation}].[{attribute}]" for attribute in foreignKeys)}, {
				# 	", ".join(f"[{foreign_relation}].[{foreign_attribute}]" for foreign_relation, foreign_attribute in foreignConnections)
				# 	} FROM [{relation}] {
				# 	" ".join(f"LEFT JOIN [{foreign_relation}] ON [{foreign_relation}].[{foreign_attribute}]" for foreign_relation, foreign_attribute in foreignConnections)}"""
				# 	if (locationInfo):
				# 		selectCommand += f" WHERE ({locationInfo})"

				# 	print("\n@getValue.2.1", selectCommand)
				# 	print("@getValue.2.2", self.executeCommand(selectCommand), "\n")
				...
				results_catalogue[relation] = result
	...
	def renameForeignUse(self, oldUser, newUser):
		"""Renames the keys given to this relation

		Example Input: renameForeignUse(oldUser, newUser)
		"""

		removeList = []
		for _relation in self.yieldKey(None, self.forigenKeys_used):
			for _attribute in self.yieldKey(None, self.forigenKeys_used[_relation]):
				for _index in self.yieldKey(None, self.forigenKeys_used[_relation][_attribute]):
					for _user in self.yieldKey(oldUser, self.forigenKeys_used[_relation][_attribute][_index]):
						removeList.append((_relation, _attribute, _index, _user))
						self.forigenKeys_used[_relation][_attribute][_index][newUser] += self.forigenKeys_used[_relation][_attribute][_index][_user]
	...
	def removeTrigger(self, label = None, applyChanges = None):
		...
		if (triggerList is not None):
			for trigger in self.ensure_container(triggerList):
				#Execute SQL
	...
	def createIndex(self, relation, attribute, name = None, noReplication = False):
		...
		command = f"""CREATE INDEX{["", " IF NOT EXISTS"][noReplication]} {name or f"{relation}_{'_'.join(attribute)}"} ON [{relation}] ({", ".join(f"[{item}]" for item in attribute)})"""
		self.executeCommand(command)
	...
	def getIndex(self, label = None, exclude = None):
		...
		indexList = self.executeCommand("SELECT name FROM sqlite_master WHERE type = 'index'")
		indexList = [index[0] for index in indexList if index[0] not in exclude]

		if (label is not None):
			if (label not in indexList):
				return
			else:
				return indexList[indexList.index(label)]
		return indexList
	...
	def removeIndex(self, relation = None, attribute = None, name = None, noReplication = False):
		...
		if (attribute is None):
			...
			indexList = (f"{relation}_{'_'.join(attribute)}",)

		if (indexList is not None):
			for index in self.ensure_container(indexList):
				command = f"DROP INDEX{['', ' IF EXISTS'][noReplication]} [{relation}].{index}"
	...
	def sandbox(self):
		...
		self.createRelation("supplier_groups", [{"group_id": int}, {"group_name": str}], 
			primary = {"group_id": True}, autoPrimary = False, 
			notNull = {"group_name": True}, unique = {"group_name": True})

		self.createRelation("suppliers", [{"supplier_id": int}, {"supplier_name": str}, {"group": int}], 
			primary = {"supplier_id": True}, autoPrimary = False, 
			notNull = {"supplier_name": True}, 
			foreign = {"group": ("supplier_groups", "group_name")})

		self.addTuple({"supplier_groups": [{"group_name": "Domestic"}, {"group_name": "Global"}, {"group_name": "One-Time"}]}, unique = None)
		self.addTuple({"suppliers": {"supplier_name": "Lorem", "group": "Temporary"}}, unique = None)
		# self.addTuple({"suppliers": {"supplier_name": "Ipsum", "group": "Temporary"}}, unique = None)
		self.addTuple({"suppliers": {"supplier_name": "Dolor", "group": "One-Time"}}, unique = None)
		self.addTuple({"suppliers": {"supplier_name": "Sit", "group": "Domestic"}}, unique = None)
		# assert self.getValue({"suppliers": "group"}, checkForeign = False) == (4, 4, 3, 1)

		# self.changeTuple({"suppliers": {"group": "TBA"}}, {"supplier_name": "Dolor"})
		# assert self.getValue({"suppliers": "group"}, checkForeign = False) == (4, 4, 5, 1)

		# self.addTuple({"suppliers": {"supplier_name": "Amet", "group": "Domestic"}}, unique = None)
		# assert self.getValue({"suppliers": "group"}, checkForeign = False) == (4, 4, 5, 1, 1)

		# self.changeTuple({"suppliers": {"group": "TBA"}}, {"supplier_name": "Amet"})
		# assert self.getValue({"suppliers": "group"}, checkForeign = False) == (4, 4, 5, 1, 5)

		# self.changeTuple({"suppliers": {"group": "Random"}}, {"supplier_name": "Amet"})
		# assert self.getValue({"suppliers": "group"}, checkForeign = False) == (4, 4, 5, 1, 6)

		# self.changeTuple({"suppliers": {"group": "Random 2"}}, {"supplier_name": "Amet"})
		print(self.getValue({"suppliers": "group"}, checkForeign = False))

		# self.changeTuple({"suppliers": {"group": "Random"}}, {"supplier_name": "Dolor"})
		# print(self.getValue({"suppliers": "group"}, checkForeign = False))

		# self.addTuple({"suppliers": {"supplier_name": "HP", "group": 2}}, unique = None)
		# # self.addTuple({"suppliers": {"supplier_name": "ABC Inc.", "group_id": 4}}, unique = None)
		# self.addTuple({"suppliers": {"supplier_name": "XYZ Corp", "group": 2}}, unique = None)
		# self.addTuple({"suppliers": {"supplier_name": "ABC Corp", "group_id": 3}}, unique = None)

		# self.changeTuple({"supplier_groups": {"group_id": 300}}, {"group_name": 'One-Time'})
		# quiet(self.getValue("suppliers", checkForeign = False))
		# quiet(self.getValue("suppliers", checkForeign = True))
		# quiet(self.getValue("suppliers", {}, checkForeign = True))
...
def test_sqlite():
	...
	try:
		...
		database_API.createIndex("Users", "name")
		...
		# quiet(database_API.getValue({"Other Users": "name"}, forceRelation = True, forceAttribute = True, forceTuple = True))
		# quiet(database_API.getValue({"Other Users": "name"}, forceRelation = True, forceAttribute = True))
		...
		# database_API.changeTuple({"Users": "name"}, {"age": 26}, "Ipsum")
		# database_API.changeTuple({"Users": "name"}, {"age": 26}, "Dolor")
		# database_API.changeTuple({"Users": "name"}, {"age": 26}, "Ipsum")
		# database_API.changeTuple({"Users": "name"}, {"age": 26}, "Dolor")
		# database_API.changeTuple({"Users": "name"}, {"age": 26}, "Ipsum")
		# database_API.changeTuple({"Users": "name"}, {"age": 26}, "Dolor")
